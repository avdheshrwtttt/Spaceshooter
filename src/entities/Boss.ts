...
/** "Cartwheel Overlord" — a rotating ring-shaped boss themed after the Cartwheel Galaxy. */
export class Boss {
  x: number;
  y = -120;
  readonly size = 130;
  ...
  constructor(w: number, tier: number) {
    this.x = w / 2;
    this.targetY = 150;
    this.tier = tier;
    this.maxHp = 50 + tier * 22;
    this.hp = this.maxHp;
    this.speed = 80 + tier * 6;
  }

  private podPos(i: number): { x: number; y: number; angle: number } {
    const a = (TAU / PODS) * i + this.ringRot;
  ...
  private bullet(x: number, y: number, vx: number, vy: number): Bullet {
    return { x, y, vx, vy, r: 6, friendly: false, damage: 1, color: "#ff8a3d", life: 8 };
  }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    // Outer rotating wheel-and-spokes ring — the Cartwheel motif.
    ctx.save();
    ctx.rotate(this.ringRot);
    ctx.shadowColor = "#ff8a3d";
    ctx.shadowBlur = 22;
    ctx.strokeStyle = this.hitFlash > 0 ? "#fff" : "#ff8a3d";
    ctx.lineWidth = 7;
    ctx.beginPath();
    ...
    for (let i = 0; i < PODS; i++) {
      ...
      ctx.translate(x2, y2);
      ctx.shadowBlur = 12;
      ctx.fillStyle = this.hitFlash > 0 ? "#fff" : "#ffcf9e";
      ctx.beginPath();
    ...
    ctx.shadowBlur = 26;
    ctx.shadowColor = "#ff5b3d";
    const coreR = s * 0.24;
    const g = ctx.createRadialGradient(0, 0, 0, 0, 0, coreR);
    g.addColorStop(0, "#fff3c2");
    g.addColorStop(0.5, this.hitFlash > 0 ? "#fff" : "#ff5b3d");
    g.addColorStop(1, "#5c0a00");
    ctx.fillStyle = g;
    ctx.beginPath();
  ...
  drawHealthBar(ctx: CanvasRenderingContext2D, w: number) {
    ...
    ctx.shadowBlur = 0;
    ctx.strokeStyle = "rgba(255,207,158,0.7)";
    ctx.lineWidth = 1;
    ...
    ctx.fillText(`⚠ CARTWHEEL OVERLORD${this.tier > 1 ? ` · TIER ${this.tier}` : ""}`, w / 2, by - 4);
    ctx.restore();
  }
}
